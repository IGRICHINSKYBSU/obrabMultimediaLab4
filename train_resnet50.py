...
Original file is located at
    https://colab.research.google.com/drive/1TqVmBWMJcXrMtef58pYga1hdSradS5Cq
"""
...
def augmented_train(image, label):
    noise = tf.random_normal(shape=tf.shape(image), mean=0.0, stddev=(10)/(255), dtype=tf.float32)
    image = image + noise
    # image = tf.image.random_flip_left_right(image)  
    # if random.random() > 0.5:
    #     image = tf.contrib.image.rotate(image, 30 * math.pi / 180, interpolation='BILINEAR')  # c-2
    # image = tf.image.random_brightness(image, 0.08)  # c-3
    # image = tf.image.random_contrast(image, 0.6, 1.4)  # c-3
